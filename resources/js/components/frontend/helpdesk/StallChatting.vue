  ...
  <div>
    ...
    <div id="container" class="customer">
            ...
            <ul class="list-group aside_ul">
                <li class="aside_li list-group-item d-flex justify-content-between align-items-center" v-for="(chatUser, userIndex) in chatUserList" :key="userIndex" @click="chatWith(chatUser); focusUser(userIndex)" :class="'user_list'+userIndex">
                        
                    <img v-if="chatUser.user_photo != undefined" style="width:45px; height:45px; margin-left:1%;" class="" :src="url+'/'+chatUser.user_photo" alt="">
                    <small class="aside_username" style="margin-left: 27%; position:absolute;">{{ chatUser.user_name }} &nbsp; <i class="status" :class="chatUser.online_status == 1?'green':''"></i></small>
                    <small class="missed_call_count" style="margin-left: 27%;  position:absolute;margin-top: 18%;" v-if="chatUser.missed_call_count > 0">{{chatUser.missed_call_count}} missed call</small>
                    <!-- v-if="chatUser.missed_call_count > 0" -->
                    <span class="badge badge-primary badge-pill" v-if="chatUser.message_reading > 0"> {{chatUser.message_reading}} </span>
        ...
        <main class="customer-main">
            <header>
                <div class="image">
                    <img v-if="chattingUser.user_photo != undefined" style="width:45px;height:45px;" :src="url+'/'+chattingUser.user_photo" alt="No Image">
                    <img v-else :src="userImage+'user-default.png'" style="width:45px;height:45px;" alt="">
                </div>
                <div class="user-name">
                    <h2>{{ chattingUser.user_name != null?chattingUser.user_name:'No User' }}</h2>
                    <h4>{{ chattingUser.online_status == 1?'online':'offline' }}</h4>
                </div>
                <!-- <img src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/1940306/ico_star.png" alt=""> -->
                <div class="tools">
                    <span v-if="chattingUser.user_name !=null && this.user.user.role == 'Customer'" @click="openCallWindow();chatWith(chattingUser);"><i class="fa fa-video"></i></span>
                    
                    <span v-else @click="makeCall(chattingUser.user_id, 'video');"><i class="fa fa-video"></i></span>
                   
                    <span v-if="chattingUser.user_name !=null && this.user.user.role == 'Customer'" @click="openCallWindow();chatWith(chattingUser);"><i class="fas fa-phone" aria-hidden="true" style="transform: rotateZ(92deg);"></i></span>
                    
                    <span v-else @click="makeCall(chattingUser.user_id, 'voice');"><i class="fas fa-phone" aria-hidden="true" style="transform: rotateZ(92deg);"></i></span>
                    
                    <span  @click="closeChat"><i class="fa fa-minus"></i></span>
                    <span @click="closeChat"><i class="fa fa-times" aria-hidden="true"></i></span>
                </div>
            </header> 
            <ul id="chat" class="customer-chat" ref="chat_box"> 
                ...
                <li v-for="(msg, msgIndex) in send_message" :key="msgIndex" :class="user.user.id == msg.user_id?'me':'you'"  v-if="!msg.isComplete">
                    <div class="entete">
                    <!-- <span class="status" :class="{'green':msg.message_class === 'you', 'blue':msg.message_class === 'me'}"></span> -->
                        <!-- <h2>{{ user.user.id == msg.user_id?user.user.name:msg.user_name}}</h2> -->
                        
                    </div>
                    <!-- <div class="triangle"></div> -->

                    <img v-if="user.user.id != msg.user_id && msg.user_photo != undefined && isMessageShow != msg.id" :class="{'you':user.user.id != msg.user_id}" style="width:35px;height:35px; border-radius:50%" :src="url+'/'+msg.user_photo" alt="">
                    ...
                    <!-- Message sender image showing -->
                     <img v-if="user.user.id == msg.user_id && msg.user_photo != undefined && isMessageShow != msg.id" :class="{'me':user.user.id == msg.user_id}" style="width:35px;height:35px; border-radius:50%" :src="url+'/'+msg.user_photo" alt="">
            ...
            <footer>
                ...
                <form class="chatForm" @submit.prevent="sendMessage();scrollActionOnSubmit()" @keyup.enter="sendMessage();scrollActionOnEnter()">
...
export default {
mixins:[frmixin],
props:['closeChat', 'sendMessage','chatWith', 'send_message', 'receive_message', 'chatUserList', 'chattingUser', 'chatFormData', 'images', 'is_sent', 'openCallWindow', 'makeCall'],
...
mounted(){
    ...
    if(this.$isMobile() == true){
        this.containerWidth = 'width:100% !important'
    } else {
        this.containerWidth = 'width:'+parseInt(window.innerWidth / 2)+'px'
    }
...
methods:{
    ...
    focusUser(i){
        $('.aside_li').removeClass('focus_user')
    ...
    deleteSingleMessage(data, message, index){
        ...
        axios.post(this.url+'/api/delete-chat-message', data)
        .then(res => {
            ...
            axios.get(this.url+'/api/get-pair-message?user_id='+data.receiver_id+'&receiver_id='+data.user_id+'&stall_id='+data.stall_id)
            .then(response => {
    ...
    deleteAllConversation(data){
        axios.post(this.url+'/api/delete-all-conversation', data)
        .then(res => {
